    ...
    <main className="relative w-full h-screen flex flex-col overflow-hidden text-elfy-green selection:bg-elfy-green selection:text-black font-mono">
      ...
      <div className={`flex-1 flex flex-col h-full transition-all duration-500 ${bootState === 'active' ? 'opacity-100' : 'opacity-0'}`}>
        ...
        >
          ...
          {/* LEFT COL */}
          <div className="md:col-span-4 flex flex-col gap-4 md:gap-6 h-auto md:h-full">
            {/* Identity: Tall on mobile, fits on desktop */}
            <GlassPanel title="IDENTITY_CORE" className="flex-1 min-h-[400px] md:min-h-0">
            ...
            <GlassPanel title="SOCIAL_UPLINK" className="h-40 md:h-48 shrink-0">
               <SocialRow />
            </GlassPanel>
          </div>

          {/* RIGHT COL */}
          <div className="md:col-span-8 flex flex-col gap-4 md:gap-6 h-auto md:h-full">
            
            {/* 35% Height on Desktop, 200px on Mobile */}
            <GlassPanel title="LATEST_LOGS" className="h-48 md:h-[35%] min-h-[200px] md:min-h-0">
            ...
            {/* Bottom Row */}
            <div className="h-auto md:h-[65%] flex flex-col md:flex-row gap-4 md:gap-6">
              
              {/* Art Grid: Tall on mobile, fits on desktop */}
              <GlassPanel title="ART_DB" className="flex-1 min-h-[450px] md:min-h-0">
                 <LiveArtGrid />
              </GlassPanel>

              {/* Holo Comm: Very tall on mobile, 35% width on desktop */}
              <GlassPanel title="HOLO_COMM" className="w-full md:w-[35%] shrink-0 min-h-[600px] md:min-h-0">
                 <HoloCommLog />
              </GlassPanel>
            </div>
          </div>
