...
export class AudioSynthesizer {
  public static async generate(recipe: SoundDef): Promise<AudioBuffer | null> {
    ...
    if (!AudioContextClass || !OfflineContextClass) return null;
    ...
    if (recipe.duration < 10.0) mainGain.gain.exponentialRampToValueAtTime(0.01, recipe.duration);
    else mainGain.gain.setValueAtTime(recipe.volume, recipe.duration);
    ...
    if (recipe.type === 'oscillator') {
        ...
        if (recipe.frequency[1] !== recipe.frequency[0]) osc.frequency.exponentialRampToValueAtTime(recipe.frequency[1], recipe.duration);
        if (recipe.fm) {
