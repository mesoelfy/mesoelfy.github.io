...
export const SOUND_LOOKUP: Record<SoundCode, AudioKey | null> = {
  [SoundCode.NONE]: null, [SoundCode.FX_PLAYER_FIRE]: 'fx_player_fire', [SoundCode.FX_IMPACT_LIGHT]: 'fx_impact_light', [SoundCode.FX_IMPACT_HEAVY]: 'fx_impact_heavy', [SoundCode.FX_ENEMY_FIRE]: 'fx_enemy_fire', [SoundCode.UI_CLICK]: 'ui_click', [SoundCode.UI_HOVER]: 'ui_hover', [SoundCode.LOOP_DRILL]: 'loop_drill', [SoundCode.FX_REBOOT_SUCCESS]: 'fx_reboot_success', [SoundCode.FX_LEVEL_UP]: 'fx_level_up', [SoundCode.LOOP_HEAL]: 'loop_heal', [SoundCode.LOOP_WARNING]: 'loop_warning', [SoundCode.FX_TELEPORT]: 'fx_teleport', [SoundCode.FX_EXHAUST_SIZZLE]: 'fx_exhaust_sizzle', [SoundCode.FX_PLAYER_DEATH]: 'fx_player_death'
};

export const FX_LOOKUP: Record<FXCode, VFXKey | null> = {
  [FXCode.NONE]: null, [FXCode.EXPLOSION_PURPLE]: 'EXPLOSION_PURPLE', [FXCode.EXPLOSION_YELLOW]: 'EXPLOSION_YELLOW', [FXCode.EXPLOSION_RED]: 'EXPLOSION_RED', [FXCode.IMPACT_WHITE]: 'IMPACT_WHITE', [FXCode.IMPACT_RED]: 'IMPACT_RED', [FXCode.DRILL_SPARKS]: 'DRILL_SPARKS', [FXCode.HUNTER_RECOIL]: 'HUNTER_RECOIL', [FXCode.CLASH_YELLOW]: 'CLASH_YELLOW', [FXCode.EXPLOSION_PURPLE_DIR]: 'EXPLOSION_PURPLE_DIR', [FXCode.EXPLOSION_YELLOW_DIR]: 'EXPLOSION_YELLOW_DIR', [FXCode.EXPLOSION_RED_DIR]: 'EXPLOSION_RED_DIR', [FXCode.REBOOT_HEAL]: 'REBOOT_HEAL', [FXCode.PURGE_BLAST]: 'PURGE_BLAST', [FXCode.ENGINE_FLARE]: 'ENGINE_FLARE'
};

export const getSoundCode = (key: string): SoundCode => {
    for (const [code, val] of Object.entries(SOUND_LOOKUP)) if (val === key) return Number(code) as SoundCode;
    return SoundCode.NONE;
};

export const getFXCode = (key: string): FXCode => {
    for (const [code, val] of Object.entries(FX_LOOKUP)) if (val === key) return Number(code) as FXCode;
    return FXCode.NONE;
};
...
export class FastEventBusImpl implements IFastEventService {
  ...
  public spawnFX(code: FXCode, x: number, y: number, angle: number = 0) {
    this.emitRaw(FastEventType.SPAWN_FX, code, Math.round(x * FLOAT_SCALAR), Math.round(y * FLOAT_SCALAR), Math.round(angle * FLOAT_SCALAR));
  }
  public playSound(code: SoundCode, pan: number = 0) {
    this.emitRaw(FastEventType.PLAY_SOUND, code, Math.round(pan * FLOAT_SCALAR));
  }
  public camShake(amount: number) {
    this.emitRaw(FastEventType.CAM_SHAKE, Math.round(amount * FLOAT_SCALAR));
  }
