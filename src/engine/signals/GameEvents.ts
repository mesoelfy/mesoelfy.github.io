...
export type FXVariant = 
  | 'EXPLOSION_PURPLE' | 'EXPLOSION_YELLOW' | 'EXPLOSION_RED'
  | 'IMPACT_WHITE' | 'IMPACT_RED' | 'DRILL_SPARKS' | 'HUNTER_RECOIL'
  | 'CLASH_YELLOW' | 'REBOOT_HEAL' | 'PURGE_BLAST' | 'ENGINE_FLARE'
  | 'EXPLOSION_PURPLE_DIR' | 'EXPLOSION_YELLOW_DIR' | 'EXPLOSION_RED_DIR';
...
export interface GameEventPayloads {
  ...
  [GameEvents.GLOBAL_STATE_SYNC]: {
    ...
    isZenMode: boolean;
    graphicsMode: string;
    debugFlags: { godMode: boolean; panelGodMode: boolean; peaceMode: boolean; timeScale: number; };
    weaponState: { spitter: SpitterState; sniffer: SnifferState; };
  };
}
