...
export class ChargeMechanic extends BTNode {
  ...
  tick(entity: Entity, context: AIContext): NodeState {
    ...
    if (typeof state.data.chargeProgress === 'undefined') state.data.chargeProgress = 0;
    if (state.data.chargeProgress < 1.0) {
        state.data.chargeProgress += context.delta / this.duration;
        if (state.data.chargeProgress >= 1.0) {
            state.data.chargeProgress = 1.0; state.current = AI_STATE.READY;
            context.playSound('ui_optimal', 0); return NodeState.SUCCESS;
        }
        state.current = AI_STATE.CHARGING; return NodeState.RUNNING;
    }
    return NodeState.SUCCESS;
  }
}
...
export class FireDaemonShot extends BTNode {
  ...
  tick(entity: Entity, context: AIContext): NodeState {
    ...
    const dx = target.x - transform.x; const dy = target.y - transform.y;
    const dist = Math.sqrt(dx*dx + dy*dy);
    let dirX = 1, dirY = 0;
    if (dist > 0.001) { dirX = dx / dist; dirY = dy / dist; }
    context.spawnProjectile(transform.x + (dirX * 0.5), transform.y + (dirY * 0.5), dirX * this.speed, dirY * this.speed, this.damage, 'DAEMON_ORB', entity.id as number);
    context.spawnFX('IMPACT_WHITE', transform.x, transform.y); context.playSound('fx_teleport', transform.x);
    state.data.chargeProgress = 0; state.data.lastFireTime = context.time; state.current = AI_STATE.ORBIT;
    return NodeState.SUCCESS;
  }
}
...
export class DaemonAim extends BTNode {
  ...
  tick(entity: Entity, context: AIContext): NodeState {
    ...
    let targetAngle = 0;
    const isCharged = state.data.chargeProgress >= 1.0; const hasEnemy = target.id !== null && target.id !== undefined;
    if (isCharged && hasEnemy) { const dx = target.x - transform.x; const dy = target.y - transform.y; targetAngle = Math.atan2(dy, dx); } else { targetAngle = Math.atan2(-transform.y, -transform.x); }
...
export class HasTargetLock extends BTNode {
  tick(entity: Entity, context: AIContext): NodeState {
    const target = entity.getComponent<TargetData>(ComponentType.Target);
    if (target && target.id) return NodeState.SUCCESS;
