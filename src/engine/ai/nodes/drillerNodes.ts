...
export class DrillAttack extends BTNode {
  ...
  tick(entity: Entity, context: AIContext): NodeState {
    ...
    let destX = target.x; let destY = target.y;
    ...
    const dx = destX - transform.x; const dy = destY - transform.y;
    ...
    if (!state.timers.drillAudio || state.timers.drillAudio <= 0) {
        context.playSound('loop_drill', transform.x); state.timers.drillAudio = 0.25;
    } else { state.timers.drillAudio -= context.delta; }

    if (!state.timers.drillDmg || state.timers.drillDmg <= 0) {
        const damage = combat ? combat.damage : 1;
        if (target.type === 'PANEL' && target.id) {
            context.damagePanel(target.id as PanelId, damage, { source: { x: transform.x, y: transform.y } });
            state.timers.drillDmg = this.interval;
        } else { state.timers.drillDmg = this.interval; }
    } else { state.timers.drillDmg -= context.delta; }
