# PERSONAL SITE
add favorite things like music
...
For patron panel, have total views shown. set up google analytics


Esper Elfy - 115_Marrow_Reptilian_Serpent
https://mesoelfy.github.io/



We need to decide what happens when the user clicks a dashboard image. What if it makes the clicked image appear as the first image in the modal, and then the subsequent images that appear in the mason grid all have the same tag as the clicked image turned on?


"donate" character portrait packs to different sites that can use them. Include the transparent PNGs and full res frames.

If a user has refreshed/gone to the site three times, a pop-up appears inviting people to join a free monthly newsletter that features my favorites posts of the month.

Hunter projectiles shouldn't instantly disappear on contact. Something visually interesting.

I'd like to make sure that someone scraping my site, either an AI indepently or a human using AI will receive a message from me with my email contact@stevencasteel.com. a "senpai notice me" message.



I'd like to donate my square character portraits to a bunch of different game assets sites. What are my options? Itch.io I"m assuming is a good one. Unity and Unreal marketplaces.


HOW TO GET MORE OUT OF THESE PORTRAITS


LICENSE
CC0 Public Domain. Do whatever you'd like with them.

PRO TIP
Feed to an AI as a style reference with a prompt like
"Create a clothed aquatic avian female version of this character with a smug expression."



The only way to get 4K output from Nano Banana Pro is via the API at AI Studio.
...
First batch also links every other batch.
Do batches of 30.
